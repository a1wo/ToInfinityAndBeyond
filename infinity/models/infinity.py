...
class Infinity(nn.Module):
    ...
    def autoregressive_infer_cfg_2(
        ...
        cond_BD_or_gss_orig, cond_BD_orig, ca_kv_orig, bs = process_prompt(label_B_or_BLT, negative_label_B_or_BLT)
        if edit:
            print("EDITING PROMPT")
            cond_BD_or_gss_edit, cond_BD_edit, ca_kv_edit, _ = process_prompt(edit__label_B_or_BLT, negative_label_B_or_BLT)
        else:
            cond_BD, ca_kv = cond_BD_orig, ca_kv_orig
            cond_BD_or_gss = cond_BD_or_gss_orig
        ...
        for si, pn in enumerate(scale_schedule):   # si: i-th segment
            ...
            if edit:
                if si > edit__si:
                    cond_BD_or_gss, cond_BD, ca_kv = cond_BD_or_gss_edit, cond_BD_edit, ca_kv_edit
                else:
                    cond_BD_or_gss, cond_BD, ca_kv = cond_BD_or_gss_orig, cond_BD_orig, ca_kv_orig
